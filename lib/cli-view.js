...
const view = (config, state) =>
  ...
  - stream with ${c.blue.bold(config.shardsIds.length)} shards: ${c.dim.grey(
    config.shardsIds.join(', ')
  )}
  - received so far ${c.blue.bold(state.count)} records (${c.grey.dim(
    _.map(config.shardsIds, si => state.shardCount[si] || 0).join(', ')
  )})
${
    !state.fileStreaming
      ? ''
    ...
    !state.lastJsonRecord
      ...
      : !deltaCount
        ...
        : `${deltaCount} new records, last being:\n${util.inspect(
            _.omit(state.lastJsonRecord, ['content']),
