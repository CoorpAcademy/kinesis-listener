...
const view = (config, state) =>
    `${c.bold(`${c.red(timeBasedSpin(timeSpinner))} Listening ${c.blue.underline(config.kinesisStream)} kinesis`)}
  - stream with ${c.blue.bold(config.shardsIds.length) } shards: ${c.dim.grey(config.shardsIds.join(', '))}
  - received so far ${c.blue.bold(state.count)} records (${
        c.grey.dim(_.map(config.shardsIds, si => state.shardCount[si] || 0).join(', '))})
${!config.fileStreaming ? '' :
        `  - ${config.fileStreamingMessage} ${c.cyan(timeBasedSpin(timeSpinner, 0.5))
            } ${config.fileStreamingFile ? c.dim.underline(config.fileStreamingFile) : ''}
`}${!state.count ? '' :
  ...
  - last received record ${
    moment(state.timestampLastReceived).fromNow()
            } (at ${c.dim.grey(moment(state.timestampLastReceived).format(config.dateFormat))}) :
