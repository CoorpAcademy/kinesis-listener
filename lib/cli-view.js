...
const view = detailedViews => (config, state) => {
  const statuses = _.compact(_.map(v => v(config, state), detailedViews));
  // TODO perform wraping
  const details = _.join(
    '\n',
    _.map(status => indentString(status, 2).replace(/^ /, '-'), statuses)
  );
  return `${c.bold(
